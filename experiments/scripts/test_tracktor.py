...
from experiments.relation_attention.FRCNN_FPN_relation import FRCNN_FPN_atten
...
from tracktor.utils import img2video
from tracktor.utils import video2img
...
def main(tracktor, reid, _config, _log, _run):
    if tracktor['dataset'] == 'showvideo':
        _log.info("Generating images.")
        video2img()
    ...
    for seq in dataset:
        ...
        if tracktor['write_images']:
            if tracktor['dataset'] == 'showvideo':
                print(seq)
                plot_sequence(results, seq, 'output/mysite/showvideo_result_img/')
            else:
    ...
    if tracktor['dataset'] == 'showvideo':
        _log.info("Generating result video.")
        img2video()
