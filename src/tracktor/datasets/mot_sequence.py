...
###------------------------------------------------------------------------------------------------------------------###
class showvideoSequence(Dataset):
    def __init__(self, seq_name=None, dets='', vis_threshold=0.0,
        ...
        self._img_dir = 'data/mysite/showvideo_img/'
    ...
    def write_results(self, all_tracks, output_dir):
        ...
        file = osp.join(output_dir, 'showvideo_result.txt')
...
class MOT17Sequence(Dataset):
    ...
    def __init__(self, seq_name=None, dets='', vis_threshold=0.0,
        ...
        # 图片是否存在，添加 '-DPM'， 在MOT17文件夹中判断
        if seq_name is not None:
            assert seq_name+'-DPM' in self._train_folders or seq_name in self._test_folders, \
    ...
    def _sequence(self):
        seq_name = self._seq_name
        # seq_name+'-DPM' 多加一个检测器名字
        if seq_name+'-DPM' in self._train_folders:
        ...
        # 改为 elif
        elif seq_name+'-DPM' in self._test_folders:
        ...
        config_file = osp.join(seq_path+'-DPM', 'seqinfo.ini')
        ...
        # 加检测器名称
        imDir = osp.join(seq_path+'-DPM', imDir)
        gt_file = osp.join(seq_path+'-DPM', 'gt', 'gt.txt')
